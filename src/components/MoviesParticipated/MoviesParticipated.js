...
import { getMoviesParticipated } from "~/apiServices/apiServices";
import config from "~/config";
import Images from "../Images/Images";
import RateFilm from "../RateFilm/RateFilm";

function MoviesParticipated({ id }) {
  const [moviesParticipated, setMoviesParticipated] = useState([]);
  useEffect(() => {
    const fetchApi = async () => {
      const res = await getMoviesParticipated(id);
    ...
    return (
      <div className="mt-[32px]">
        ...
        <div className="flex relative w-full flex-wrap">
          {moviesParticipated.map((item, index) => {
            return (
              <Link
                key={index}
                to={`/movie/${item.id}`}
